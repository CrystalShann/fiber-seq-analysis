#!/bin/bash
#SBATCH --job-name=fire_freq_spans
...
#SBATCH --time=8:00:00
#SBATCH --array=1-4
#SBATCH --output=/project/spott/cshan/fiber-seq/results/logs/fire_freq_spans_%A_%a.out
#SBATCH --error=/project/spott/cshan/fiber-seq/results/logs/fire_freq_spans_%A_%a.err
...
if [ -s "$spans" ] && [ -s "${spans}.tbi" ]; then
    ...
    for chrom in $CHROMS; do
        # Extract primary alignments and convert them to BED. Remove secondary
        # (0x100) and supplementary (0x800) alignments to avoid duplicate names.
