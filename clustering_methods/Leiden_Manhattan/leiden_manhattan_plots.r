...
# PDF links require no image conversion or preview artifacts.
report_gallery <- function(result_paths, include_haplotype = FALSE, output_dir = LCL_TOP10_ROOT) {
  files <- c("fiberseq_haplotype_example", "fiberseq_umap_overview", "fiberseq_profiles_composition", "knn_graph")
  labels <- c("Combined haplotype example", "UMAP and cluster proportions", "Profiles and within-cluster composition",
    "KNN graph colored by Leiden cluster")
  rows <- lapply(names(result_paths), function(rid) {
    ...
    cells <- lapply(seq_along(files), function(i) {
      pdf <- file.path(plot_dir, paste0(files[i], ".pdf"))
      stopifnot(file.exists(pdf), startsWith(pdf, paste0(output_dir, "/")))
      htmltools::tags$td(htmltools::tags$a(
        href = report_url(pdf), labels[i]))
    })
...
plot_knn_graph <- function(result, main = NULL, seed = NULL, vertex_size = 1.5,
  ...
  counts <- table(nodes$cluster)
  labels <- paste0(clusters, " (n=", as.integer(counts[clusters]), ")")
  if (is.null(main)) main <- paste(c(result$region$annotation,
    "Manhattan KNN graph colored by Leiden cluster"), collapse = "\n")
  ...
  ggplot2::ggplot() +
    ...
    ggplot2::geom_point(data = nodes, ggplot2::aes(layout_x, layout_y, color = cluster),
      size = vertex_size) +
    ggplot2::scale_color_manual(values = colors, breaks = clusters, labels = labels, drop = FALSE) +
    ggplot2::coord_equal() +
    ggplot2::labs(title = main, subtitle = paste(settings, collapse = " | "), color = "Leiden cluster") +
...
# Input footprints must include the requested nucleosome and FiberHMM tracks.
...
# The RCC viewer serves files through a query parameter, so child links must
# carry their full filesystem path rather than resolve relative to /spott/.
...
write_result_index <- function(output_dir = LCL_TOP10_ROOT, include_haplotype = TRUE) {
  output_dir <- lcl_output_path(output_dir)
  paths <- readRDS(file.path(output_dir, "summary tables", "fiberseq_result_paths.rds"))
  main_paths <- readRDS(file.path(output_dir, "summary tables", "result_paths.rds"))
  stopifnot(all(names(main_paths) %in% names(paths)))
  paths <- paths[names(main_paths)]
  lines <- c("# Top 10 heterozygous AS-FIRE regions", "",
    "Saved main Leiden assignments; exactly 2,000 bp around each lead SNP. Only resolved phased heterozygous reads are included.",
    "Allele annotations use actual focal bases. Sample bars sum to 100% within each cluster.", "")
  files <- c("fiberseq_haplotype_example", "fig1_read_footprints", "fig2_occupancy_by_cluster", "heatmap_m6a_footprints",
    "methylation_proportion_by_cluster", "cluster_sample_composition", "fiberseq_umap_overview", "fiberseq_profiles_composition",
    "knn_graph")
  for (rid in names(paths)) {
    plot_dir <- file.path(dirname(dirname(paths[[rid]])), "plots")
    urls <- vapply(file.path(plot_dir, paste0(files, ".pdf")), report_url, character(1))
    lines <- c(lines, paste0("## ", rid), "",
      paste0("- [", files, "](", urls, ")"), "")
  }
...
run_fiberseq_report <- function(regions, asfire_file, sample_table, phasing_root,
                                    output_dir = LCL_TOP10_ROOT, seed = 1L, reuse_cache = TRUE) {
  output_dir <- lcl_output_path(output_dir)
  stopifnot(nrow(regions) == 10L, all(regions$width == 2000L),
  ...
  write_result_index(output_dir)
  paths
}

# Replot saved top-ten results without rebuilding m6A matrices or main clusters.
refresh_saved_plots <- function(output_dir = LCL_TOP10_ROOT,
...
render_report <- function(input, ...) {
  ...
  rmarkdown::render(input, output_file = "leiden_LCL_top10_asfire_het_2kb.html", output_dir = root,
